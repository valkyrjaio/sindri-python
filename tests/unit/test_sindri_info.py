...
from sindri.constant.sindri_info import SindriInfo
...
def test_version_has_the_version_format() -> None:
    assert VERSION_PATTERN.fullmatch(SindriInfo.VERSION)


def test_version_build_date_time_has_the_build_date_time_format() -> None:
    assert VERSION_BUILD_DATE_TIME_PATTERN.fullmatch(SindriInfo.VERSION_BUILD_DATE_TIME)
